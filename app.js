// ============================================================
// LIQUORBELLE — MASTER APP.JS
// One file — all pages.
// Automatically detects which page it's on
// and runs only the relevant code.
// ============================================================
...
if (document.getElementById('categoryGrid')) {
  ...
  function qvSelectVariant(flavour, size, price, discount) {
  ...
  (function initShop() {
    ...
    function renderActiveFilterPills() {
      ...
      if (currentFilters.category !== 'all') {
        var catName = (categories.find(function(c) { return c.cat === currentFilters.category; }) || {}).name || currentFilters.category;
    ...
    function clearSearchPill() { setSearch('');
      applyFilters(); }

    function clearPricePill() { currentFilters.priceMin = null;
      currentFilters.priceMax = null;
      currentPage = 1;
      applyFilters(); }

    function clearSalePill() { currentFilters.onSale = false;
      currentPage = 1;
      applyFilters(); }
    ...
    function updateTitleBar() {
      ...
      if (currentFilters.category !== 'all') {
        catName = (categories.find(function(c) { return c.cat === currentFilters.category; }) || {}).name || currentFilters.category;
      }
      if (currentFilters.search) {
    ...
    function goToProduct(id, name, price, image, capacity) {
    ...
    window.clearSearchPill = clearSearchPill;
    window.clearPricePill = clearPricePill;
    window.clearSalePill = clearSalePill;
  })();
}
...
if (document.getElementById('productImage')) {
  (function initProductDetail() {
    ...
    function renderProductDetails() {
      ...
      for (var i = 0; i < variants.length; i++) {
        ...
        html += '<button class="variant-btn" ' +
          ...
          (flavourDisplay ? '<span class="flavour">' + escapeHtml(flavourDisplay) + '</span>' : '') +
    ...
    function selectVariant(flavour, size, price, discount) {
    ...
    function increaseQty() { if (quantity < 10) { quantity++;
    ...
    function renderDontForgetSuggestions() {
    ...
    function updateDeliveryProgress() {
      ...
      var hasBig = items.some(function(i) { return ['750ml', '1L', '1.5L'].indexOf(i.capacity) > -1; });
      var dp = document.getElementById('deliveryProgress');
      if (!dp) return;
      if (hasBig) {
        dp.className = 'delivery-progress unlocked';
        dp.innerHTML = '<strong>Free delivery unlocked!</strong><div class="delivery-progress-bar"><div class="delivery-progress-fill" style="width:100%;"></div></div>';
      } else {
        dp.className = 'delivery-progress';
        dp.innerHTML = 'Add any 750ml/1L bottle to unlock <strong>FREE delivery</strong> (save KES 150)<div class="delivery-progress-bar"><div class="delivery-progress-fill" style="width:25%;"></div></div>';
      }
    }

    // Override updateCartUI for product detail (add delivery progress)
    ...
    // Also update delivery progress on cart changes
    var origAddToCart = addToCart;
    addToCart = function(id, name, price, capacity) {
      origAddToCart(id, name, price, capacity);
      updateDeliveryProgress();
    };
  })();
}
                  ...
                  if (statusData.status === 'paid') {
                    ...
                    fetch(API_BASE + '/api/db/orders', {
                      ...
                      .then(function() {
                        ...
                        // Save last order for guest tracking
                        try {
...
if (document.getElementById('trackEmail')) {
  (function initTrackOrders() {
    ...
    // Expose to global
    window.trackByEmail = trackByEmail;
    window.trackByOrderId = trackByOrderId;
...
if (document.getElementById('productsTab')) {
  ...
  (function initAdminFull() {
    ...
    // Category helpers (already defined globally)
    var CATEGORY_LABELS = {
    ...
    document.querySelector('.header-logout')?.addEventListener('click', logoutAdmin);

    // ---- Product CRUD ----
    function toggleVariantOther(select) {
    ...
    function toggleEditVariantOther(select) {
      var td = select.parentElement;
    ...
    function updateEditVariantOptions(category) {
    ...
    async function bulkImport() {
      ...
      try {
        ...
        for (var i = 1; i < lines.length; i++) {
          ...
          for (var k = 1; k <= 10; k++) {
            var flavourKey = 'Flavour' + k,
              sizeKey = 'Size' + k,
              priceKey = 'Price' + k,
              discountKey = 'Discount' + k;
    ...
    function sortProducts(arr) {
      var s = document.getElementById('sortSelect')?.value || 'newest';
    ...
    function applySort() { renderProductsAdmin(); }
    ...
    async function loadOrdersAdmin() {
      ...
      try {
        ...
        var data = await res.json();
        if (data.success) {
          allOrders = data.orders.sort(function(a, b) { return new Date(b.created_at) - new Date(a.created_at); });
          applyFiltersAdmin();
        }
      } catch(e) {
        allOrders = [];
    ...
    async function deleteOrderAdmin(id) {
      ...
      var idx = -1,
        deleted = null;
      for (var i = 0; i < filteredOrders.length; i++) {
        if (filteredOrders[i]._id === id) { idx = i;
          deleted = filteredOrders[i]; break; }
      }
      if (idx !== -1) {
...
if (document.getElementById('cashierLoginModal')) {
  (function initAdminOrders() {
    var adminToken = localStorage.getItem('liquorbelle_cashier_token');
    ...
    var FALLBACK_IMG_ADMIN = '';

    function getAuthHeadersOrders() {
      return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + adminToken };
    }

    function showToastOrders(msg, isError) {
      var t = document.getElementById('toast');
      if (!t) return;
      t.innerText = msg;
    ...
    document.querySelector('.header-logout')?.addEventListener('click', logoutOrders);

    function formatPhoneOrders(phone) {
      if (!phone) return '';
    ...
    function findProductImageOrders(productName, productId) {
      if (!productName && !productId) return FALLBACK_IMG_ADMIN;
      if (productId) {
        for (var i = 0; i < products.length; i++) {
          if (products[i]._id === productId) { if (products[i].image) return products[i].image; break; }
        }
      }
      if (productName) {
        for (var i = 0; i < products.length; i++) {
          if (products[i].name && products[i].name.toLowerCase() === productName.toLowerCase()) {
            if (products[i].image) return products[i].image;
            break;
          }
        }
        for (var i = 0; i < products.length; i++) {
      ...
      return FALLBACK_IMG_ADMIN;
    }

    function applyFiltersOrders() {
      currentDateFilter = document.getElementById('dateFilter').value;
      currentStatusFilter = document.getElementById('statusFilter').value;
      ...
      if (currentDateFilter !== 'all') {
        var daysAgo = new Date(Date.now() - parseInt(currentDateFilter) * 24 * 60 * 60 * 1000);
    ...
    function renderStatsOrders() {
      var total = filteredOrders.length;
      var pending = filteredOrders.filter(function(o) { return o.status === 'pending'; }).length;
      var paid = filteredOrders.filter(function(o) { return o.status === 'paid'; }).length;
      var delivered = filteredOrders.filter(function(o) { return o.status === 'delivered'; }).length;

      var periodText = '';
      if (currentDateFilter === 'all') periodText = 'All Time';
      else if (currentDateFilter === '1') periodText = 'Last 24 Hours';
      else if (currentDateFilter === '7') periodText = 'Last 7 Days';
      else if (currentDateFilter === '30') periodText = 'Last 30 Days';
      else if (currentDateFilter === '90') periodText = 'Last 90 Days';
      else if (currentDateFilter === '365') periodText = 'Last Year';

      var statusText = '';
      if (currentStatusFilter === 'all') statusText = 'Orders';
      else if (currentStatusFilter === 'pending') statusText = 'Pending';
      else if (currentStatusFilter === 'paid') statusText = 'Paid';
      else if (currentStatusFilter === 'delivered') statusText = 'Delivered';
    ...
    function renderOrdersOrders() {
      var filtered = filteredOrders.slice();
      if (searchTerm) {
        filtered = filteredOrders.filter(function(o) {
          return (o.customer_name && o.customer_name.toLowerCase().includes(searchTerm)) ||
      ...
      for (var i = 0; i < filtered.length; i++) {
        ...
        var phoneRaw = o.phone || '';
        var cleanPhone = formatPhoneOrders(phoneRaw);
        ...
        for (var j = 0; j < items.length; j++) {
          var item = items[j];
          var productImg = findProductImageOrders(item.product_name || item.name, item.product_id);
        ...
        var actionsHtml = '';
        if (o.status === 'pending') {
          actionsHtml = '<button class="btn btn-sm btn-green" onclick="markPaidOrders(\'' + o._id + '\')" ' + (isProcessing ? 'disabled' : '') + '><i class="ph ph-check-circle"></i> ' + (isProcessing ? '<span class="loading-spinner"></span>' : 'Mark Paid') + '</button>';
        } else if (o.status === 'paid') {
          actionsHtml = '<button class="btn btn-sm btn-gold" onclick="markDeliveredOrders(\'' + o._id + '\')" ' + (isProcessing ? 'disabled' : '') + '><i class="ph ph-truck"></i> ' + (isProcessing ? '<span class="loading-spinner"></span>' : 'Mark Delivered') + '</button>';
    ...
    async function loadProductsOrders() {
      try {
        var res = await fetch(API_BASE + '/api/db/products');
        var data = await res.json();
        if (data.success) products = data.products;
    ...
    async function loadOrdersOrders() {
      ...
      try {
        ...
        if (res.status === 429) {
          showToastOrders('⚠️ Too many requests. Please wait...', true);
          return;
        }
        var data = await res.json();
        if (data.success) {
    ...
    function manualRefresh() {
      showToastOrders('Refreshing orders...');
      loadOrdersOrders();
    }

    // ---- Mark Paid ----
    async function markPaidOrders(id) {
      if (pendingActions.has(id)) return;

      var orderIndex = -1;
      for (var i = 0; i < filteredOrders.length; i++) {
        if (filteredOrders[i]._id === id) { orderIndex = i; break; }
      }
      if (orderIndex === -1) return;
      var originalOrder = JSON.parse(JSON.stringify(filteredOrders[orderIndex]));
      ...
      try {
        ...
        if (res.status === 429) {
          showToastOrders('⚠️ Server busy, retrying...', true);
          setTimeout(function() { markPaidOrders(id); }, 3000);
        ...
        if (res.status === 401 || res.status === 403) {
          ...
          if (autoRefreshInterval) clearInterval(autoRefreshInterval);
          document.getElementById('cashierLoginModal').style.display = 'flex';
          showToastOrders('Session expired. Please login again.', true);
          pendingActions.delete(id);
          var revertIndex = -1;
          for (var i = 0; i < filteredOrders.length; i++) {
            if (filteredOrders[i]._id === id) { revertIndex = i; break; }
          }
          if (revertIndex !== -1) filteredOrders[revertIndex] = originalOrder;
          var allIdx = -1;
          for (var i = 0; i < allOrders.length; i++) {
            if (allOrders[i]._id === id) { allIdx = i; break; }
          }
          if (allIdx !== -1) allOrders[allIdx] = originalOrder;
          renderOrdersOrders();
          renderStatsOrders();
          return;
        }
        ...
        if (res.ok) {
          showToastOrders('✅ Order marked as PAID');
          var updateIndex = -1;
          for (var i = 0; i < filteredOrders.length; i++) {
            if (filteredOrders[i]._id === id) { updateIndex = i; break; }
          }
          if (updateIndex !== -1) filteredOrders[updateIndex].status = 'paid';
          var allUpdateIdx = -1;
          for (var i = 0; i < allOrders.length; i++) {
            if (allOrders[i]._id === id) { allUpdateIdx = i; break; }
          }
          if (allUpdateIdx !== -1) allOrders[allUpdateIdx].status = 'paid';
          pendingActions.delete(id);
          renderOrdersOrders();
          renderStatsOrders();
          setTimeout(function() { loadOrdersOrders(); }, 500);
        } else {
          var revertIndex = -1;
          for (var i = 0; i < filteredOrders.length; i++) {
            if (filteredOrders[i]._id === id) { revertIndex = i; break; }
          ...
          renderOrdersOrders();
          renderStatsOrders();
          showToastOrders('Error updating order', true);
        }
      } catch(e) {
        var revertIndex = -1;
        for (var i = 0; i < filteredOrders.length; i++) {
        ...
        renderOrdersOrders();
        renderStatsOrders();
        showToastOrders('Network error', true);
      }
    }

    // ---- Mark Delivered ----
    async function markDeliveredOrders(id) {
      if (pendingActions.has(id)) return;
      ...
      try {
        ...
        if (res.status === 429) {
          showToastOrders('⚠️ Server busy, retrying...', true);
          setTimeout(function() { markDeliveredOrders(id); }, 3000);
          pendingActions.delete(id);
          renderOrdersOrders();
          return;
        }
        ...
        if (res.ok) {
          showToastOrders('🚚 Order marked as DELIVERED');
          var updateIndex = -1;
          for (var i = 0; i < filteredOrders.length; i++) {
            if (filteredOrders[i]._id === id) { updateIndex = i; break; }
          }
          if (updateIndex !== -1) filteredOrders[updateIndex].status = 'delivered';
          var allUpdateIdx = -1;
          for (var i = 0; i < allOrders.length; i++) {
            if (allOrders[i]._id === id) { allUpdateIdx = i; break; }
          }
          if (allUpdateIdx !== -1) allOrders[allUpdateIdx].status = 'delivered';
          pendingActions.delete(id);
          renderOrdersOrders();
          renderStatsOrders();
          setTimeout(function() { loadOrdersOrders(); }, 500);
    ...
    // ---- Initialize ----
    async function initAfterLoginOrders() {
      await loadProductsOrders();
      await loadOrdersOrders();
      if (autoRefreshInterval) clearInterval(autoRefreshInterval);
      autoRefreshInterval = setInterval(function() {
        if (adminToken) loadOrdersOrders();
      }, 10000);
    }

    async function initOrders() {
      adminToken = localStorage.getItem('liquorbelle_cashier_token');
      if (adminToken) {
        document.getElementById('cashierLoginModal').style.display = 'none';
        await initAfterLoginOrders();
      } else {
        document.getElementById('cashierLoginModal').style.display = 'flex';
        setTimeout(function() { document.getElementById('cashierPassword').focus(); }, 300);
      }
    }
...
// ============================================================
// INITIAL UPDATE: Cart & Wishlist Modal
// ============================================================
...
console.log('🚀 LiquorBelle — Master app.js loaded');